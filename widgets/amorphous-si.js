...
function fftshift2d(a, N) {
  const h = N >> 1,
    o = new Float64Array(N * N);
  for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) o[((r + h) % N) * N + ((c + h) % N)] = a[r * N + c];
  return o;
}
...
function turboColor(t) {
  ...
  const g = Math.max(
    0,
    Math.min(255, Math.round(23.31 + t * (557.33 + t * (1225.33 + t * (-5765.73 + t * (8642.97 + t * -4136.93))))))
  );
  const b = Math.max(
    0,
...
function countCrystalline(atoms, N, rBond) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (nn === 4 && !bad) {
      ...
      my /= 4;
      mz /= 4;
      if (mx * mx + my * my + mz * mz < thresh2) count++;
    }
  }
  return count;
}
...
function findBondsGrid(atoms, N, rBond) {
  ...
  for (let i = 0; i < N; i++) {
    const xi = atoms[i * 3],
      ...
      zi = atoms[i * 3 + 2];
    const cx = Math.floor(xi / cellSize) + gH,
      cy = Math.floor(yi / cellSize) + gH,
      cz = Math.floor(zi / cellSize) + gH;
  ...
  for (let i = 0; i < N; i++) {
    if (nnList[i].length === 4) {
      ...
      for (const j of nnList[i]) {
        mx += atoms[j * 3] - atoms[i * 3];
        my += atoms[j * 3 + 1] - atoms[i * 3 + 1];
        mz += atoms[j * 3 + 2] - atoms[i * 3 + 2];
      }
      mx /= 4;
      my /= 4;
      mz /= 4;
  ...
  const bonds = [];
  for (let i = 0; i < N; i++)
    for (const j of nnList[i]) if (j > i) bonds.push([i, j, crystalline[i] && crystalline[j] ? 1 : 0]);
...
function buildNNListGrid(atoms, N, rMax) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const cx = Math.floor(xi / cellSize) + gH,
      cy = Math.floor(yi / cellSize) + gH,
      cz = Math.floor(zi / cellSize) + gH;
    for (let dx = -nC; dx <= nC; dx++) {
      const nx = cx + dx;
      if (nx < 0 || nx >= gD) continue;
      for (let dy = -nC; dy <= nC; dy++) {
        const ny = cy + dy;
        if (ny < 0 || ny >= gD) continue;
        for (let dz = -nC; dz <= nC; dz++) {
          const nz = cz + dz;
          if (nz < 0 || nz >= gD) continue;
          const ci = (nx * gD + ny) * gD + nz;
          for (let k = off[ci]; k < off[ci + 1]; k++) {
      ...
      for (let iz = -M; iz <= M; iz++)
        for (const b of basis) {
          const x = (ix + b[0]) * a,
            y = (iy + b[1]) * a,
            z = (iz + b[2]) * a;
          if (x * x + y * y + z * z <= R2) atoms.push([x, y, z]);
        }
  return atoms;
}
...
function generateDisorderOps(rng, numOps, sphereR, maxRotR, minRotR) {
  ...
  for (let i = 0; i < numOps; i++) {
    ...
    let cx, cy, cz;
    do {
      cx = (rng.random() * 2 - 1) * maxC;
      cy = (rng.random() * 2 - 1) * maxC;
      cz = (rng.random() * 2 - 1) * maxC;
    } while (cx * cx + cy * cy + cz * cz > maxC * maxC);
    const th = rng.random() * 2 * Math.PI,
      ph = Math.acos(2 * rng.random() - 1);
    const ax = Math.sin(ph) * Math.cos(th),
      ay = Math.sin(ph) * Math.sin(th),
      az = Math.cos(ph);
...
function rotatePoint(px, py, pz, ax, ay, az, ang) {
  const c = Math.cos(ang),
    ...
    d = px * ax + py * ay + pz * az;
  return [
    px * c + (ay * pz - az * py) * s + ax * d * (1 - c),
    py * c + (az * px - ax * pz) * s + ay * d * (1 - c),
    pz * c + (ax * py - ay * px) * s + az * d * (1 - c),
  ];
}
...
function precomputeStates(crystal, ops, nSteps, minDist) {
  const N = crystal.length,
    states = new Array(nSteps + 1);
  states[0] = new Float32Array(N * 3);
  for (let i = 0; i < N; i++) {
    states[0][i * 3] = crystal[i][0];
    states[0][i * 3 + 1] = crystal[i][1];
    states[0][i * 3 + 2] = crystal[i][2];
  }
  const opsPS = ops.length / nSteps;
  for (let step = 1; step <= nSteps; step++) {
    states[step] = new Float32Array(states[step - 1]);
    const a = states[step],
      s0 = Math.floor((step - 1) * opsPS),
      s1 = Math.floor(step * opsPS);
    for (let oi = s0; oi < s1; oi++) {
      const op = ops[oi];
      for (let i = 0; i < N; i++) {
        const dx = a[i * 3] - op.cx,
          dy = a[i * 3 + 1] - op.cy,
          dz = a[i * 3 + 2] - op.cz;
...
function findActiveAtoms(atoms, N, minDist) {
  ...
  for (let p = 0; p < pairI.length; p++) {
    ...
    if (conflictCount[i] >= conflictCount[j]) {
      active[i] = 0;
    } else {
      active[j] = 0;
    }
...
function compute3Body(atoms, N, nnList, rNNmax, rMax, dr, dtDeg, sphereR, active) {
  const r2NN = rNNmax * rNNmax,
    dt = (dtDeg * Math.PI) / 180;
  const thetaBins = Math.ceil(Math.PI / dt),
    rBins = Math.ceil(rMax / dr) + 1;
  ...
  for (let ri = 1; ri < rBins; ri++) {
    let s = 0;
    for (let ti = 0; ti < thetaBins; ti++) s += body3[ti * rBins + ri];
    const ex = s / thetaBins;
    if (ex > 0) for (let ti = 0; ti < thetaBins; ti++) body3[ti * rBins + ri] /= ex;
  }
  ...
  for (let ti = 0; ti < thetaBins; ti++)
    for (let ri = 0; ri < rBins; ri++) {
      let s = 0,
        w = 0;
      for (let dti = -kT; dti <= kT; dti++) {
        const tii = ti + dti;
        if (tii < 0 || tii >= thetaBins) continue;
        for (let dri = -kR; dri <= kR; dri++) {
          const rii = ri + dri;
          if (rii < 0 || rii >= rBins) continue;
          const ww = Math.exp(-(dti * dti) / (2 * sT * sT) - (dri * dri) / (2 * sR * sR));
          s += body3[tii * rBins + rii] * ww;
          w += ww;
        }
      }
...
function computeDiffraction(atoms, N, fftSize, pixelSize, azimuth, elevation, semiangle, lambda, active) {
  ...
  const sigma = 0.3,
    sig2 = (2 * sigma * sigma) / (pixelSize * pixelSize);
  ...
  for (let i = 0; i < N; i++) {
    ...
    const x1 = x * ca - z * sa,
      z1 = x * sa + z * ca,
      y1 = y * ce - z1 * se;
    const ax = x1 / pixelSize + half,
      ay = y1 / pixelSize + half;
    if (ax < 1 || ax >= fftSize - 1 || ay < 1 || ay >= fftSize - 1) continue;
    const ix0 = Math.max(0, Math.floor(ax - drawR)),
      ix1 = Math.min(fftSize - 1, Math.ceil(ax + drawR));
    const iy0 = Math.max(0, Math.floor(ay - drawR)),
      iy1 = Math.min(fftSize - 1, Math.ceil(ay + drawR));
  ...
  const aRe = new Float64Array(fftSize * fftSize),
    aIm = new Float64Array(fftSize * fftSize);
  const semiRad = semiangle * 0.001;
  const qMaxPx = (semiRad / lambda) * fftSize * pixelSize,
    qMaxPx2 = qMaxPx * qMaxPx;
  for (let iy = 0; iy < fftSize; iy++)
    for (let ix = 0; ix < fftSize; ix++) {
      const qx = ix < half ? ix : ix - fftSize,
        qy = iy < half ? iy : iy - fftSize;
  ...
  const eRe = fftshift2d(exitRe, fftSize),
    eIm = new Float64Array(fftSize * fftSize);
...
function renderBody3ToCanvas(ctx, w, h, b3) {
  ...
  for (let py = 0; py < h; py++) {
    const ti = Math.floor((1 - py / h) * thetaBins);
    for (let ppx = 0; ppx < w; ppx++) {
      const ri = Math.floor((ppx / w) * rBins);
...
function renderDPToCanvas(ctx, w, h, dp, fftSize, gamma, maxI) {
  ...
  if (maxI <= 0) maxI = 1;
  for (let py = 0; py < h; py++) {
    const fi = Math.floor((py / h) * fftSize);
    for (let ppx = 0; ppx < w; ppx++) {
      const fj = Math.floor((ppx / w) * fftSize);
...
function render({ model, el }) {
  ...
  setTimeout(async () => {
    ...
    // cannot accidentally bind to elements from another widget instance.
    ...
    const gSlid = $("gamma"),
      ...
      dmVal = $("dmv"),
      rBtn = $("rb");
    const b3Ctx = b3C.getContext("2d"),
      grCtx = grC.getContext("2d"),
      scCtx = scC.getContext("2d"),
      dpCtx = dpC.getContext("2d");
    ...
    for (let s = 0; s <= numRawSteps; s += sI) {
      cF[s] = countCrystalline(states[s], nAtoms, rBond) / nc0;
    }
    cF[numRawSteps] = countCrystalline(states[numRawSteps], nAtoms, rBond) / nc0;
    for (let s = 0; s <= numRawSteps; s++)
      if (s % sI !== 0 && s !== numRawSteps) {
        const lo = Math.floor(s / sI) * sI,
          hi = Math.min(lo + sI, numRawSteps);
        cF[s] = cF[lo] + (cF[hi] - cF[lo]) * ((s - lo) / (hi - lo));
      }
    const remap = new Int32Array(101),
      remapPct = new Int32Array(101);
    for (let p = 0; p <= 100; p++) {
      const tgt = 1 - p / 100;
      let best = 0,
        bestD = 2;
      for (let s = 0; s <= numRawSteps; s++) {
        const d = Math.abs(cF[s] - tgt);
        if (d < bestD) {
          bestD = d;
          best = s;
        }
      ...
      remap[p] = best;
      remapPct[p] = Math.round(cF[best] * 100);
    }
    ...
    function vacuumPeakIntensity(semiVal) {
      ...
      const half = fftSize / 2,
        sR = semiVal * 0.001;
      const qMP = (sR / lambda) * fftSize * pixelSize,
        qMP2 = qMP * qMP;
      for (let iy = 0; iy < fftSize; iy++)
        for (let ix = 0; ix < fftSize; ix++) {
          const qx = ix < half ? ix : ix - fftSize,
    ...
    function runDP() {
      cDP = computeDiffraction(getA(), nAtoms, fftSize, pixelSize, az, elev, semi, lambda, null);
      needDP = false;
    }
    ...
    function rBody3() {
      ...
      const w = b3C.width,
        h = b3C.height;
      renderBody3ToCanvas(b3Ctx, w, h, cBody3);
      b3Ctx.fillStyle = "#888";
      b3Ctx.font = "9px sans-serif";
      // Angle labels
      b3Ctx.fillText("0°", 2, h - 3);
      b3Ctx.fillText("180°", 2, 10);
      // 90° mark
      const y90 = Math.round((1 - 90 / 180) * h);
      b3Ctx.strokeStyle = "rgba(255,255,255,0.3)";
      b3Ctx.setLineDash([2, 3]);
      b3Ctx.beginPath();
      b3Ctx.moveTo(0, y90);
      b3Ctx.lineTo(w, y90);
      b3Ctx.stroke();
      b3Ctx.fillStyle = "#aaa";
      b3Ctx.fillText("90°", w - 28, y90 - 2);
      // 109.5° mark
      const tY = Math.round((1 - 109.5 / 180) * h);
      b3Ctx.strokeStyle = "rgba(255,255,255,0.5)";
      b3Ctx.setLineDash([3, 3]);
      b3Ctx.beginPath();
      b3Ctx.moveTo(0, tY);
      b3Ctx.lineTo(w, tY);
      b3Ctx.stroke();
      b3Ctx.setLineDash([]);
      b3Ctx.fillStyle = "#fff";
      b3Ctx.fillText("109.5°", w - 44, tY - 2);
    }
    function rGR() {
      if (!cGR) return;
      const w = grC.width,
      ...
      for (let r = 0; r <= 5; r++) {
        const px = Math.round((r / rMax) * w);
        ctx.fillText(`${r}`, px + 1, h - 1);
      }
      ctx.fillText("radius (Å)", w - 50, h - 1);
    }
    function rDP() {
      if (cDP) renderDPToCanvas(dpCtx, dpC.width, dpC.height, cDP, fftSize, gamma, dpMaxFrac * vacuumI);
    }
    ...
    function rScene() {
      ...
      ctx.fillRect(0, 0, W, H);
      const proj = makeProjection(az, elev, 200),
        cx = W / 2,
        cy = H / 2,
        sc = Math.min(W, H) / (sphereRadius * 2.56);
      ...
      const pI = new Array(nAtoms);
      for (const p of prjAll) pI[p.i] = p;

      // Probe cone (vertical)
      const pHA = semi * 0.001,
        pE = sphereRadius * 2,
        pTR = pE * Math.tan(pHA) * sc * 6;
      const pTY = cy - pE * sc,
        pBY = cy + pE * sc;
      ctx.save();
      ...
      ctx.beginPath();
      ctx.moveTo(cx - pTR, pTY);
      ctx.lineTo(cx, cy);
      ctx.lineTo(cx + pTR, pTY);
      ctx.closePath();
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx - pTR, pBY);
      ctx.lineTo(cx + pTR, pBY);
      ctx.closePath();
      ctx.fill();
      ctx.restore();
      ...
      if (cBonds) {
        ...
        ctx.beginPath();
        for (const [i, j, cr] of cBonds.bonds) {
          if (!cr) continue;
          const a = pI[i],
            b = pI[j];
      ...
      for (let bin = 0; bin < nB; bin++) {
        const lo = Math.floor((bin / nB) * nVisible),
          hi = Math.floor(((bin + 1) / nB) * nVisible);
        const alpha = 0.15 + 0.55 * (bin / (nB - 1)),
          r = 0.8 + 0.4 * (bin / (nB - 1));
        if (cBonds) {
          ...
          for (let k = lo; k < hi; k++) {
            const p = prjAll[k];
            if (!cBonds.crystalline[p.i]) continue;
            ctx.moveTo(p.sx + r, p.sy);
            ctx.arc(p.sx, p.sy, r, 0, 6.2832);
          }
          ctx.fill();
          ctx.fillStyle = `rgba(140,140,150,${alpha * 0.4})`;
          ctx.beginPath();
          for (let k = lo; k < hi; k++) {
            const p = prjAll[k];
            if (cBonds.crystalline[p.i]) continue;
            ctx.moveTo(p.sx + r * 0.6, p.sy);
            ctx.arc(p.sx, p.sy, r * 0.6, 0, 6.2832);
          }
          ctx.fill();
        }
      }
      ...
      ctx.fillText(`${nAtoms} atoms`, 8, H - 8);
      if (cBonds) {
        let nc = 0;
        for (let i = 0; i < nAtoms; i++) if (cBonds.crystalline[i]) nc++;
        ctx.fillStyle = "rgba(80,200,255,0.7)";
        ctx.fillText(`${nc} crystalline`, 8, H - 20);
      }
    }
    ...
    let drag = false,
      ...
      ly = 0,
      dpTO = null;
    scC.addEventListener("pointerdown", (e) => {
      drag = true;
      lx = e.clientX;
      ly = e.clientY;
      scC.setPointerCapture(e.pointerId);
    });
    scC.addEventListener("pointermove", (e) => {
      if (!drag) return;
      az -= (e.clientX - lx) * 0.01;
      elev += (e.clientY - ly) * 0.01;
      lx = e.clientX;
      ly = e.clientY;
      rScene();
      if (dpTO) clearTimeout(dpTO);
      dpTO = setTimeout(() => {
        needDP = true;
        runDP();
        rDP();
    ...
    scC.addEventListener("pointerup", () => {
      drag = false;
      needDP = true;
      runDP();
      rDP();
    });
    scC.addEventListener(
      "touchstart",
      (e) => {
    ...
    scC.addEventListener(
      ...
      (e) => {
        ...
        az -= (e.touches[0].clientX - lx) * 0.01;
        elev += (e.touches[0].clientY - ly) * 0.01;
        lx = e.touches[0].clientX;
        ly = e.touches[0].clientY;
        rScene();
      },
    ...
    let aTO = null;
    dSlid.addEventListener("input", () => {
      dLevel = parseInt(dSlid.value);
      dVal.textContent = `${remapPct[dLevel]}% cryst`;
      needFull = true;
      needDP = true;
      if (aTO) clearTimeout(aTO);
      cBonds = findBondsGrid(getA(), nAtoms, rBond);
      rScene();
      aTO = setTimeout(() => renderAll(), 200);
    });
    // Auto-scale DP max with semiangle using calibration points (log-log interpolation)
    // 0.05 mrad → 0.01, 1.0 mrad → 0.03, 4.0 mrad → 0.5
    const dpCalSemi = [0.05, 1.0, 4.0, 10.0],
      dpCalMax = [0.01, 0.03, 0.5, 1.1];
    const dpCalLogS = dpCalSemi.map(Math.log10),
      dpCalLogM = dpCalMax.map(Math.log10);
    ...
    sSlid.addEventListener("input", () => {
      semi = parseFloat(sSlid.value);
      sVal.textContent = `${semi.toFixed(2)} mrad`;
      vacuumI = vacuumPeakIntensity(semi);
      updateDPMaxFromSemi();
      rScene();
      needDP = true;
      runDP();
    ...
    gSlid.addEventListener("input", () => {
      gamma = parseFloat(gSlid.value);
      gVal.textContent = gamma.toFixed(2);
      rDP();
    });
    dmSlid.addEventListener("input", () => {
      dpMaxFrac = Math.pow(10, parseFloat(dmSlid.value));
      dmVal.textContent = dpMaxFrac < 0.01 ? dpMaxFrac.toExponential(1) : dpMaxFrac.toFixed(3);
      rDP();
    });
    ...
    function stopRot() {
      rotating = false;
      if (rotId) cancelAnimationFrame(rotId);
      rotId = null;
      rBtn.classList.remove("active");
      rBtn.innerHTML = "&#x25B6; Rotate";
      needDP = true;
      runDP();
      rDP();
    }
    function startRot() {
      ...
      function tick(now) {
        ...
        last = now;
        elev += dt * 0.4; // rotate around y-axis
        dpT += dt;
        rScene();
        if (dpT > 0.6) {
          dpT = 0;
        ...
        rotId = requestAnimationFrame(tick);
      }
      rotId = requestAnimationFrame(tick);
    }
