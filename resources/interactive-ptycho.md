...
The simulation here uses a 300 kV beam with a 33 mrad convergence semiangle and a strongly defocused probe (the defocus slider spans 200 angstroms of overfocus to 200 angstroms of underfocus, with C1 equal to minus the defocus). The sample is a five-fold twinned decahedral nanoparticle with triangular 111 surface facets, embedded in an amorphous carbon substrate that fills the field of view. Every diffraction pattern in the 15 by 15 scan is computed live in your browser with the multislice algorithm, which alternates between transmission through each thin slice and Fresnel propagation between slices.
...
Because the depth resolution of ptychography is limited to roughly two times the wavelength divided by the square of the convergence angle, about 36 angstroms for these conditions, features smear along the beam direction across neighboring slices. You can see this directly in the reconstructed slice stack: the flat nanoparticle and the substrate blur across several slices, including the empty padding slices above and below the sample.
